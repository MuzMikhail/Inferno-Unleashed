...
#pragma warning( default : 26813 )
	...
	SlotProc<T>&
	SetProc( std::string proc_name, std::function<void()> func, ProcTiming timing = ProcTiming::Update, Priority prio = Priority::NORMAL )
	{
		auto& proc = GetProc<T>( proc_name, timing );
		proc.SetProc( proc_name, timing, prio, func );
		return proc;
	...
	SlotProc<void>&
	SetProc( std::string proc_name, std::function<void()> func, ProcTiming timing = ProcTiming::Draw, Priority prio = Priority::NORMAL )
	{
...
private:
	...
	CEREAL_SAVELOAD( arc, ver )
	{
		...
		arc( CEREAL_NVP( name_default_ ), CEREAL_NVP( status_.get() ), CEREAL_NVP( update_timings_ ), CEREAL_NVP( proc_timings_ ) );
