...
void Scene::Base::setProc( ComponentPtr component, SlotProc<T> slot )
{
	...
	{
		if constexpr( std::is_same<T, float>{} )
			slot.SetProc( slot.GetName(), slot.GetTiming(), slot.GetPriority(), BindComponentUpdate( slot.GetTiming(), component ) );
		else
			slot.SetProc( slot.GetName(), slot.GetTiming(), slot.GetPriority(), BindComponentProc( slot.GetTiming(), component ) );
	}
...
bool Scene::Base::Save( const std::string_view filename )
{
	...
	std::ofstream file( ".\\data\\_save\\" + name + ".txt" );
	if( !file )
		return false;
...
void Scene::checkSerialized( ObjectPtr obj )
{
	...
	for( auto& proc : obj->update_timings_ )
	{
		...
		assert( p.GetProc() != nullptr && "SetProcで追加した処理がSerializeされていません。InitSerialize()で初期化してください." );
	}
...
{
	// オブジェクトPICK
	//if( IsMouseOn( MOUSE_INPUT_LEFT ) && !ImGui::IsWindowHovered( ImGuiHoveredFlags_AnyWindow ) && !ImGuizmo::IsOver() && !IsShowMenu() )
	//{
...
ObjectPtr Scene::SelectObjectPtr()
{
	...
	if( current_scene_->GetObjectPtrVec().empty() )
		return nullptr;
