...
public:
	...
	bool   IsColliding( const float3& position );
	float3 GetSurfaceNormal( const float3& position );
...
#pragma warning( disable : 26813 )
		...
		if( grp == CollisionGroup::ETC )
			return "ETC";
		if( grp == CollisionGroup::SKELET )
			return "SKELET";
		if( grp == CollisionGroup::PORTAL )
			return "PORTAL";
		if( grp == CollisionGroup::KEY )
			return "KEY";
