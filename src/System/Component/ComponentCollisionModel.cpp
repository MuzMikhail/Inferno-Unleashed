...
void ComponentCollisionModel::AttachToModel( bool update )
{
	if( auto mdl = GetOwner()->GetComponent<ComponentModel>() )
	{
		...
		else
		{
			...
			auto mat = MV1GetMatrix( ref_model_ );
			MV1SetMatrix( ref_model_, cast( mdl->GetWorldMatrix() ) );

			// モデルが存在すればその当たりを構築する(※Defaultを使用)
			MV1RefreshCollInfo( mdl->GetModel() );
...
float3 ComponentCollision::GetPosition() const
{
	// Используйте текущую матрицу трансформации для получения позиции
	return float3( collision_transform_._41, collision_transform_._42, collision_transform_._43 );
}


bool ComponentCollision::IsColliding( const float3& position )
{
	// Пример реализации для сферического коллайдера
	float3 colliderCenter = GetPosition();	 // Предполагается, что коллайдер имеет позицию
	float  colliderRadius = 2.5f;			 // Радиус коллайдера

	float distance = length( position - colliderCenter );
	return distance <= colliderRadius;
}
float3 ComponentCollision::GetSurfaceNormal( const float3& position )
{
	// Пример реализации для сферического коллайдера
	float3 colliderCenter = GetPosition();	 // Предполагается, что коллайдер имеет позицию
	float3 normal		  = normalize( position - colliderCenter );
	return normal;
}
